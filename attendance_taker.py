...
predictor = dlib.shape_predictor ('f:/face/data_dlib/shape_predictor_68_face_landmarks.dat')

# Dlib Resnet Use Dlib resnet50 model to get 128D face descriptor
face_reco_model = dlib.face_recognition_model_v1("f:/face/data_dlib/dlib_face_recognition_resnet_model_v1.dat")
...
create_table_sql = f"CREATE TABLE IF NOT EXISTS {table_name} (id INTEGER PRIMARY KEY AUTOINCREMENT, students_id INTEGER,  time TEXT, date DATE, present BOOLEAN, UNIQUE(students_id, date))"
...
class face_recognizer:
    ...
    def attendance(self, name):
        ...
        cursor.execute("SELECT * FROM attendance JOIN student ON students_id = student.id WHERE name = ? AND date = ?", (name, current_date))
        ...
        if existing_entry:
            ...
            current_time = datetime.datetime.now().strftime('%H:%M:%S')
            student_id_query = "SELECT id FROM student WHERE name = ?"
            cursor.execute(student_id_query, (name,))
            student_id = cursor.fetchone()[0]

            attendance_insert_query = "INSERT INTO attendance (students_id, date, present) VALUES (?, ?, ?)"
            cursor.execute(attendance_insert_query, (student_id, current_date, current_time))
    ...
    def process(self, stream):
        ...
        if self.get_face_database():
            while stream.isOpened():
                ...
                # 4.  Update the face name list in the last frame
                ...
                # 6.2  If cnt of faces changes, 0->1 or 1->0 or ...
                    ...
                    # 6.2.2 / Face cnt increase: 0->1, 0->2, ..., 1->2, ...
                        ...
                        for k in range(len(faces)):
                            ...
                            # 6.2.2.4 / Find the one with the minimum e distance
