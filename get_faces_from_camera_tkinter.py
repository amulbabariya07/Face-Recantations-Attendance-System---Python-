...
class Face_Register:
    def __init__(self):
        self.current_frame_faces_cnt = 0  # Count for counting faces in the current frame
        self.existing_faces_cnt = 0  # Count for counting saved faces
        self.ss_cnt = 0  # Count for screen shots

        self.conn = sqlite3.connect('attendance2.db')
        ...
        self.input_enrollment_no = tk.Entry(self.frame_right_info,text="")  # Create a new Entry for Enrollment No
        self.input_parent_email = tk.Entry(self.frame_right_info,text="")  # Create a new Entry for Parent's Email
        self.input_name_char = ""
        self.input_enrollment_no_char = ""
        self.input_parent_email_char = ""
        ...
        self.cap = cv2.VideoCapture(0)  # Get a video stream from the camera

        # self.cap = cv2.VideoCapture("test.mp4")  # Input local video

        # Create a course selection dropdown in Step 6
        self.course_label = tk.Label(self.frame_right_info, text="Select Course:")
        self.course_combo = ttk.Combobox(self.frame_right_info, values=["Bsc (Cyber Security)", "Bsc (Data Science)", "Bsc (CA & IT)"])
    ...
    #print("-------------->>>>>>>>>>>>>>>>>>>>>>>>>>>><<<<<<<<<<<<<")
        ...
        # Modify the GUI_get_input_name method to get Enrollment No
    def GUI_get_input_name(self):
        self.input_name_char = self.input_name.get()
        self.input_enrollment_no_char = self.input_enrollment_no.get()  # Get Enrollment No
        self.input_parent_email_char = self.input_parent_email.get()  # Get Parent's Email
        self.create_face_folder()
        self.label_cnt_face_in_database['text'] = str(self.existing_faces_cnt)
        name = self.input_name.get()
        enrollment_no = self.input_enrollment_no.get()
        parent_email = self.input_parent_email.get()
        course = self.course_combo.get()

        if name and enrollment_no and parent_email and course:
            # Insert the user data into the database
            self.cursor.execute("INSERT INTO student (name, parents_email, enrollment_no, course) VALUES (?, ?, ?, ?)",
                                (name, parent_email, enrollment_no, course))
            self.conn.commit()
            self.create_face_folder()
            self.label_cnt_face_in_database['text'] = str(self.existing_faces_cnt)
        else:
            self.log_all["text"] = "Please fill in all the required fields."
    ...
    def GUI_info(self):
        ...
        tk.Button(self.frame_right_info, text='Save', command=self.GUI_get_input_name).grid(row=8, column=2, padx=5)
        ...
        # Step 4: Enter Enrollment No
        tk.Label(self.frame_right_info,
                    font=self.font_step_title,
                    text="Step 3: Enter Enrollment No").grid(row=11, column=0, columnspan=2, sticky=tk.W, padx=5, pady=20)


        tk.Label(self.frame_right_info, text="Enrollment No: ").grid(row=12, column=0, sticky=tk.W, padx=5, pady=0)
        self.input_enrollment_no.grid(row=12, column=1, sticky=tk.W, padx=0, pady=2)

        
        tk.Button(self.frame_right_info, text='Save', command=self.input_enrollment_no_char).grid(row=12, column=2, padx=5)

        # Step 5: Enter Parent's Email
        tk.Label(self.frame_right_info,
                    font=self.font_step_title,
                    text="Step 4: Enter Parent's Email").grid(row=13, column=0, columnspan=2, sticky=tk.W, padx=5, pady=20)

                # Add labels and Entry widgets for Parent's Email
        tk.Label(self.frame_right_info, text="Parent's Email: ").grid(row=14, column=0, sticky=tk.W, padx=5, pady=0)
        self.input_parent_email.grid(row=14, column=1, sticky=tk.W, padx=0, pady=2)

        tk.Button(self.frame_right_info, text='Save', command=self.input_parent_email_char).grid(row=14, column=2, padx=5)

        #tk.Button(self.frame_right_info,
                    #text='Save',
                    #command=self.GUI_get_input_name).grid(row=10, column=2, padx=5)

        # Step 6: Choose a course using a dropdown menu
        tk.Label(self.frame_right_info,
                    font=self.font_step_title,
                    text="Step 5: Choose Course").grid(row=15, column=0, columnspan=2, sticky=tk.W, padx=5, pady=20)

        self.course_label.grid(row=16, column=0, sticky=tk.W, padx=5, pady=0)
        self.course_combo.grid(row=16, column=1, sticky=tk.W, padx=0, pady=2)

        tk.Button(self.frame_right_info, text='Save', command=self.GUI_get_input_name).grid(row=15, column=2, padx=5)

        # tk.Button(self.frame_right_info,
        #             text='Save',
        #             command=self.GUI_get_input_name).grid(row=14, column=2, padx=5)
        
        tk.Button(self.frame_right_info,
                  text='Save current face',
                  command=self.save_current_face).grid(row=17, column=0, columnspan=3, sticky=tk.W)

        # Show log in GUI
        self.log_all.grid(row=18, column=0, columnspan=20, sticky=tk.W, padx=5, pady=20)
    ...
    def pre_work_mkdir(self):
        ...
        # Create 'data_faces_from_camera' directory if it doesn't exist
