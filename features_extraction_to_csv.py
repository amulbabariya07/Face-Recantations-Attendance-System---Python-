...
#  Get face landmarks
predictor = dlib.shape_predictor('D:/face/data_dlib/shape_predictor_68_face_landmarks.dat')


#  Use Dlib resnet50 model to get 128D face descriptor
face_reco_model = dlib.face_recognition_model_v1("D:/face/data_dlib/dlib_face_recognition_resnet_model_v1.dat")
...
class FaceFeaturesExtractor:
    def __init__(self):
        ...
        self.predictor = dlib.shape_predictor('D:/face/data_dlib/shape_predictor_68_face_landmarks.dat')
        self.face_reco_model = dlib.face_recognition_model_v1("D:/face/data_dlib/dlib_face_recognition_resnet_model_v1.dat")
    ...
    def return_person_info(self, person_id):
        conn = sqlite3.connect('attendance2.db')
        cursor = conn.cursor()
        cursor.execute("SELECT name, enrollment_no, parents_email, course FROM student WHERE id = ?", (person_id,))
    ...
    def main(self):
        ...
        person_list = os.listdir(self.path_images_from_camera)
        person_list.sort()

        with open("data/features_all.csv", "w", newline="") as csvfile:
            writer = csv.writer(csvfile)
            for person in person_list:
                logging.info("%sperson_%s", self.path_images_from_camera, person)
                features_mean_personX = self.return_features_mean_personX(self.path_images_from_camera + person)
